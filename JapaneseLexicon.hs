...
jumanPos2Cat daihyo ct caseframe 
  ...
  | T.isPrefixOf "名詞:サ変名詞"     ct  = [(N, commonNounSR daihyo)] ++ (constructVerb daihyo caseframe [VS] [Stem])
  ...
  | T.isPrefixOf "接続詞" ct = [(((defS anyPos [Term,Pre,Imper] `SL` defS anyPos [Term,Pre,Imper]) `BS` defS anyPos [Term]), (Lam (Lam (Lam (Sigma (App (Var 2) (Lam Top)) (App (Var 2) (Var 1)))))))]
  | T.isPrefixOf "接頭辞:イ形容詞" ct   = [((defS [Aauo] [Stem] `BS` NP [Ga]) `SL` (defS [Aauo] [Stem] `BS` NP [Ga]), id)]
  | T.isPrefixOf "接頭辞:ナ形容詞" ct   = [((defS [Nda] [Stem] `BS` NP [Ga]) `SL` (defS [Nda] [Stem] `BS` NP [Ga]), id)]
  | T.isPrefixOf "名詞性名詞助数辞" ct  = [(N,commonNounSR daihyo)]
  | T.isPrefixOf "名詞性特殊接尾辞" ct  = [(N,commonNounSR daihyo)]
  | T.isPrefixOf "名詞性名詞接尾辞" ct  = [(N,commonNounSR daihyo)]
  ...
  -- T.isPrefixOf "感動詞" t   = [(defS [Exp] [Term], id)]
...
constructPredicate daihyo cpos = [(defS cpos [Stem] `BS` NP [Ga], predSR daihyo)]
