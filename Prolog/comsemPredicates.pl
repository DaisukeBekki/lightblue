...
basicFormula(F):-
   ...
   \+ memberList(Symbol,[not,and,imp,app,or,some,all,one,two,three,lam,eq]),
   simpleTerms(Args).
...
substitute(Term,Var,Formula,Result):-
   ...
   memberList(Functor,[lam,all,some,one,two,three]), !, 
   (
    Exp==Var, !, 
    Result=Formula
   ; 
...
user:portray(one(X,F)):- bbmode(infix), !, write('one '), write_term(X,[numbervars(true)]), tab(1), print(F).
user:portray(two(X,F)):- bbmode(infix), !, write('two '), write_term(X,[numbervars(true)]), tab(1), print(F).
user:portray(three(X,F)):- bbmode(infix), !, write('three '), write_term(X,[numbervars(true)]), tab(1), print(F).
