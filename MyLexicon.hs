...
myLexicon = concat $ [
 ...
 -- 助動詞（否定）
  ...
  mylex ["ね"] "new" (defS anyPos [Term] `BS` defS anyPos [Term]) id,
  mylex ["よ"] "new" (defS anyPos [Term] `BS` defS anyPos [Term]) id,
  mylex ["さ"] "new" (defS anyPos [Term] `BS` defS anyPos [Term]) id,
  -- 句読点
