...
findArgument events caselist headwords caselistlist = case events of
  ...
  ((StartElement "argument" attribs):es) ->
    ...
    case (do
      ...
      Just (_,cas) ->
        case () of
          _ | ("ノ格" `T.isPrefixOf` cas || cas `elem` ["ガ２","デ格","ヘ格","カラ格","マデ格","ヨリ格","について","にもとづいて","にあわせて","にくわえて","にとって","において","をめぐって","にならんで","未格","無格","外の関係","修飾","時間"]) -> findArgument es caselist headwords caselistlist
