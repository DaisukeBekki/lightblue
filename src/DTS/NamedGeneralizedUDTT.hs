...
module DTS.NamedGeneralizedUDTT (
  -- * Terms and Types
  VarName(..)
  , Selector(..)
  , Preterm(..)
...
import qualified DTS.GeneralizedUDTT as DB
...
type Context a = [(VarName, Preterm a)]
...
fromDeBruijnContextLoop :: DB.Context a -> Indexed ([VarName], Context a)
...
toVerticalMathML :: Context a -> T.Text
...
data Judgment a = Judgment {
  ...
  , context :: Context DB.DTT -- ^ A context \Gamma in \Gamma \vdash M:A
