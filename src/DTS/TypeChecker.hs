...
typeInfer :: QT.TypeInfer -- | QT.Prover -> QT.ProofSearchSetting -> UDTTdB.TypeInferQuery -> ListT IO QT.TypeCheckResult
typeInfer prover pss tiq@(UDTTdB.TypeInferQuery sig ctx trm) = do
  ...
  case trm of -- sig:Signature, ctx:Context, trm:Preterm DTT, typ:Preterm DTT
    ...
    UDTTdB.Pi termA termB -> do
      diagramA <- typeCheck prover pss (UDTTdB.Judgment sig ctx termA DTTdB.Type)  -- | diagramAs :: [Tree]
      let termA' = DTTdB.trm $ node diagramA
      diagramB <- typeCheck prover pss (UDTTdB.Judgment sig (termA':ctx) termB DTTdB.Type)
    ...
    UDTTdB.App termM termN -> do
      diagramM <- typeInfer prover pss $ UDTTdB.TypeInferQuery sig ctx termM
      let termM' = DTTdB.trm $ node diagramM
          (DTTdB.Pi termA termB) = DTTdB.typ $ node diagramM
      diagramN <- typeCheck prover pss $ UDTTdB.Judgment sig ctx termN termA
    ...
    UDTTdB.Sigma termA termB -> do
      diagramA <- typeCheck prover pss $ UDTTdB.Judgment sig ctx termA DTTdB.Type
      let termA' = DTTdB.trm $ node diagramA
      diagramB <- typeCheck prover pss $ UDTTdB.Judgment sig (termA':ctx) termB DTTdB.Type
      let termB' = DTTdB.trm $ node diagramB
    ...
    UDTTdB.Unpack termP termL termM termN -> do
      diagramL <- typeInfer prover pss $ UDTTdB.TypeInferQuery sig ctx termL
      let termL' = DTTdB.trm $ node diagramL
          (DTTdB.Disj termA termB) = DTTdB.typ $ node diagramL
      diagramP <- typeCheck prover pss $ UDTTdB.Judgment sig ctx termP (DTTdB.Pi (DTTdB.Disj termA termB) DTTdB.Type)
      ...
      diagramM <- typeCheck prover pss $ UDTTdB.Judgment sig ctx termM (DTTdB.Pi termA termPM)
      diagramN <- typeCheck prover pss $ UDTTdB.Judgment sig ctx termM (DTTdB.Pi termB termPN)
      let termM' = DTTdB.trm $ node diagramM
    ...
    UDTTdB.Eq termA termM termN -> do
      diagramA <- typeCheck prover pss $ UDTTdB.Judgment sig ctx termA DTTdB.Type
      let termA' = DTTdB.trm $ node diagramA
      diagramM <- typeCheck prover pss $ UDTTdB.Judgment sig ctx termM termA'
      let termM' = DTTdB.trm $ node diagramM
      diagramN <- typeCheck prover pss $ UDTTdB.Judgment sig ctx termN termA'
      let termN' = DTTdB.trm $ node diagramN
    ...
    UDTTdB.Succ termN -> do
      diagramN <- typeCheck prover pss $ UDTTdB.Judgment sig ctx termN DTTdB.Nat
      let termN' = DTTdB.trm $ node diagramN
    ...
    --   diagramA <- typeCheck prover pss $ UDTTdB.Judgment sig ctx termA UDTTdB.Type
    ...
    --   typeCheck prover pss $ UDTTdB.Judgment sig ctx termB' UDTTdB.Type
    -- | type inference fails
    termM -> fail $ T.unpack $ T.concat [toText termM, " is not an inferable term."]

-- | The default type checker for UDTT
typeCheck :: QT.TypeChecker -- | Prover -> QT.ProofSearchSetting -> UDTTdB.Judgment -> ListT IO TypeCheckResult
typeCheck prover pss tcq@(UDTTdB.Judgment sig ctx trm typ) = do
  ...
  case (trm,typ) of -- sig:Signature, ctx:Context, trm:Preterm DTT, typ:Preterm DTT
    (UDTTdB.Lam termM, DTTdB.Pi termA termB) -> do
      diagramA <- typeCheck prover pss $ UDTTdB.Judgment sig ctx (UDTTdB.toUDTT termA) DTTdB.Type
      let termA' = DTTdB.trm $ node diagramA
      diagramM <- typeCheck prover pss $ UDTTdB.Judgment sig (termA':ctx) termM termB
      let termM' = DTTdB.trm $ node diagramM
    ...
    (UDTTdB.Pair termM termN, DTTdB.Sigma termA termB) -> do
      ...
      diagramN <- typeCheck prover pss $ UDTTdB.Judgment sig ctx termN termB'
      let termN' = DTTdB.trm $ node diagramN
    ...
    _ -> do
      diagramM <- typeInfer prover pss $ UDTTdB.TypeInferQuery sig ctx trm
