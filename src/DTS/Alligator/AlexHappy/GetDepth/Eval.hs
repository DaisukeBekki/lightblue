module DTS.Alligator.AlexHappy.GetDepth.Eval (evalNum) where
...
import qualified DTS.Alligator.AlexHappy.GetDepth.Parser as P
...
evalNum ::[S.Expr] -> String -> S.Nums
evalNum = setNum

setNum :: [S.Expr] -> String -> S.Nums
setNum expr fname= foldl updateNum def expr

updateNum:: S.Nums -> S.Expr -> S.Nums
updateNum base expr = do
  case expr of
    S.NumOf "predicates" n -> base{S.predicates=n}
    S.NumOf "atoms" n -> base{S.atoms=n}
    S.NumOf "variables" n -> base{S.variables=n}
    S.NumOf "clauses" n -> base{S.clauses=n}
    S.NumOf "clause size" n -> base{S.clauseSize=n}
    S.NumOf "functors" n -> base{S.functors=n}
    S.NumOf "Maximal term depth" n -> base{S.maximalDepth=n}
    _ -> base
